/*
	...
	Program Usage:
		aesomp encrypt threadNum {KEYSIZE=128,192,256} "ENCRYPTION_KEY" "PLAINTEXT"
		aesomp decrypt threadNum {KEYSIZE=128,192,256} "ENCRYPTION_KEY" "CIPHERTEXT"

	Linux Compilation Command: g++ -fopnemp OpenMP.cpp -o aesomp -O3
*/
...
int main(int argc, char* argv[]) {
	...
	int threadNum;			// max amount of threads that will be used
	...
	if (argc >= 2) {							// Handle program help
		if (std::string(argv[1]) == "help") {
			std::cout << "\nType \"aes help\" to show help on how to use this program (this text).\n\n";
			printHelpText();
			return 0;
		}
	}
	if (argc > 6) {						// validate argument count
		std::cerr << "Too many program arguments! Enter 'aes help' for more information." << std::endl;
		return -1;
	}
	else if (argc < 6) {
		std::cerr << "Too few program arguments! Enter 'aes help' for more information." << std::endl;
		return -1;
	}
	arg = std::string(argv[1]);					// validate program cipher mode [encrypt/decrypt]
	if (arg != "encrypt" && arg != "decrypt") {
		std::cerr << '\'' << arg << "' is an invalid program mode. Valid program modes are 'encrypt' or 'decrypt'. Enter 'aes help' for more information." << std::endl;
		return -1;
	}
	else mode = (arg == "encrypt" ? 0 : 1);

	arg = std::string(argv[2]);					// set max thread amount
	threadNum = atoi(arg.c_str());

	arg = std::string(argv[3]);					// validate keysize value
	if (arg != "128" && arg != "192" && arg != "256") {
		std::cerr << '\'' << arg << "' is an invalid keysize value. Valid keysizes are '128', '192', or '256' (bits). Enter 'aes help' for more information." << std::endl;
		return -1;
	}
	...
	arg = std::string(argv[4]);					// validate encryption key (size)
	if ((keysize == 128 && arg.size() != 16) || (keysize == 192 && arg.size() != 24) || (keysize == 256 && arg.size() != 32)) {
		...
		std::cerr << " characters (exact length) for " << keysize << " bit " << (mode == 0 ? "encryption" : "decryption") << ". Enter 'aes help' for more information." << std::endl;
		return -1;
	}
	key = arg;

	arg = std::string(argv[5]);					// validate plaintext/ciphertext
	if (arg.size() == 0) {
		std::cerr << "Invalid input.";
		std::cerr << (mode == 0 ? "Plaintext" : "Ciphertext");
		std::cerr << " cannot be blank. Enter 'aes help' for more information." << std::endl;
		return -1;
	}
	text = arg;
	...
	int index;
	char temp;
	std::string out = "";				// final plaintext/ciphertext
	...
	#pragma omp parallel num_threads(threadNum)		// Initialize parallelism for up to threadNum threads
	#pragma omp for ordered schedule(static,1) private(state)	// Parallelize the for loop, scheduling each thread only 1 chunk of the array at a time
	for (int i = 0; i < segments; i++) {			// Iteratively run cipher over blocks

			if (!mode) {	//Sequential Section start
				for (int j = 0; j < 16; ++j) {
					index = (16 * i) + j;
...
#pragma omp ordered											// Ensure updates are performed in the order they were scheduled
				//std::cout << "Thread " << omp_get_thread_num() << " has i value: " << i << std::endl;
				for (auto c : state) buffer.push_back(c);
			}
			else {
				for (int j = 0; j < 16; ++j) {
					index = (16 * i) + j;
...
#pragma omp ordered											// Ensure updates are performed in the order they were scheduled
	...
	if (!mode) {
		out = b64encode(buffer);									// base 64 encode buffer for console output
		std::cout << out << '\n';
	}
	else std::cout << out << '\n';
...
void printHelpText() {
	std::cout << "Usage:\n\
aes encrypt threads KEYSIZE \"ENCRYPTION_KEY\" \"PLAINTEXT\"\n\
aes decrypt threads KEYSIZE \"ENCRYPTION_KEY\" \"CIPHERTEXT\"\n\
eg. aes encrypt 4 128 this_is_the_key_ \"This is some random plaintext. Unlike the key, it can be of any length\".\n\
\n\
Notes:\n\
- 'aes' invokes this program.\n\
- 'threads': Defines how many threads are wanted to run the program \n\
